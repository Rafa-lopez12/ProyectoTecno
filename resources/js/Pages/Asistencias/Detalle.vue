...
const verReprogramacion = () => {
    router.visit(route('asistencias.reprogramacion', licencia.value.id))
};

// Volver
const volver = () => {
    if (asistencia.value?.inscripcion_id) {
        router.visit(route('asistencias.por-inscripcion', asistencia.value.inscripcion_id))
    } else {
        ...
        <div class="space-y-6">
            ...
            <template v-else-if="asistencia">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <button
                        @click="router.visit(route('asistencias.informe', asistenciaId))"
